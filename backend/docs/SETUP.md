...
Quick setup instructions for TalonFlow.

## Configuration Setup

TalonFlow uses a YAML configuration file that should be placed in the **project root directory**.

### Steps

1. **Navigate to project root**:
   ```bash
   cd /path/to/talon-flow
   ```
...
- **Location**: `config.yaml` should be in `talon-flow/` (project root), not `talon-flow/backend/`
...
1. `TALON_FLOW_CONFIG_PATH` environment variable (if set)
2. `backend/config.yaml` (current directory when running from backend/)
3. `talon-flow/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`talon-flow/config.yaml`).

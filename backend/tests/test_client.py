...
def client(mock_app_config):
    ...
    with patch("talonflow.client.get_app_config", return_value=mock_app_config):
...
class TestClientInit:
    ...
    def test_custom_config_path(self, mock_app_config):
        with (
            patch("talonflow.client.reload_app_config") as mock_reload,
            patch("talonflow.client.get_app_config", return_value=mock_app_config),
        ):
...
class TestConfigQueries:
    ...
    def test_list_skills(self, client):
        ...
        with patch("talonflow.skills.loader.load_skills", return_value=[skill]) as mock_load:
    ...
    def test_list_skills_enabled_only(self, client):
        with patch("talonflow.skills.loader.load_skills", return_value=[]) as mock_load:
            client.list_skills(enabled_only=True)
            mock_load.assert_called_once_with(enabled_only=True)

    def test_get_memory(self, client):
        memory = {"version": "1.0", "facts": []}
        with patch("talonflow.agents.memory.updater.get_memory_data", return_value=memory) as mock_mem:
...
class TestEnsureAgent:
    def test_creates_agent(self, client):
        ...
        with (
            patch("talonflow.client.create_chat_model"),
            patch("talonflow.client.create_agent", return_value=mock_agent),
            patch("talonflow.client._build_middlewares", return_value=[]) as mock_build_middlewares,
            patch("talonflow.client.apply_prompt_template", return_value="prompt") as mock_apply_prompt,
    ...
    def test_uses_default_checkpointer_when_available(self, client):
        ...
        with (
            patch("talonflow.client.create_chat_model"),
            patch("talonflow.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("talonflow.client._build_middlewares", return_value=[]),
            patch("talonflow.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("talonflow.agents.checkpointer.get_checkpointer", return_value=mock_checkpointer),
        ):
            client._ensure_agent(config)
    ...
    def test_injects_custom_middlewares(self, client):
        ...
        with (
            patch("talonflow.client.create_chat_model"),
            patch("talonflow.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("talonflow.client._build_middlewares", side_effect=fake_build_middlewares),
            patch("talonflow.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
        ):
            client._ensure_agent(config)
    ...
    def test_skips_default_checkpointer_when_unconfigured(self, client):
        ...
        with (
            ...
            patch("talonflow.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("talonflow.agents.checkpointer.get_checkpointer", return_value=None),
        ):
            client._ensure_agent(config)
...
class TestMcpConfig:
    def test_get_mcp_config(self, client):
        ...
        with patch("talonflow.client.get_extensions_config", return_value=ext_config):
    ...
    def test_update_mcp_config(self, client):
        ...
        try:
            ...
            with (
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("talonflow.client.get_extensions_config", return_value=current_config),
                patch("talonflow.client.reload_extensions_config", return_value=reloaded_config),
            ):
...
class TestSkillsManagement:
    ...
    def test_get_skill_found(self, client):
        ...
        with patch("talonflow.skills.loader.load_skills", return_value=[skill]):
            result = client.get_skill("test-skill")
        assert result is not None
        assert result["name"] == "test-skill"

    def test_get_skill_not_found(self, client):
        with patch("talonflow.skills.loader.load_skills", return_value=[]):
    ...
    def test_update_skill(self, client):
        ...
        try:
            ...
            with (
                patch("talonflow.skills.loader.load_skills", side_effect=[[skill], [updated_skill]]),
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("talonflow.client.get_extensions_config", return_value=ext_config),
                patch("talonflow.client.reload_extensions_config"),
            ):
    ...
    def test_install_skill(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("talonflow.skills.installer.get_skills_root_path", return_value=skills_root):
...
class TestMemoryManagement:
    def test_import_memory(self, client):
        ...
        with patch("talonflow.agents.memory.updater.import_memory_data", return_value=imported) as mock_import:
    ...
    def test_reload_memory(self, client):
        ...
        with patch("talonflow.agents.memory.updater.reload_memory_data", return_value=data):
            result = client.reload_memory()
        assert result == data

    def test_clear_memory(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("talonflow.agents.memory.updater.clear_memory_data", return_value=data):
            result = client.clear_memory()
        assert result == data

    def test_create_memory_fact(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("talonflow.agents.memory.updater.create_memory_fact", return_value=data) as create_fact:
    ...
    def test_delete_memory_fact(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("talonflow.agents.memory.updater.delete_memory_fact", return_value=data) as delete_fact:
    ...
    def test_update_memory_fact(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("talonflow.agents.memory.updater.update_memory_fact", return_value=data) as update_fact:
    ...
    def test_get_memory_config(self, client):
        ...
        with patch("talonflow.config.memory_config.get_memory_config", return_value=config):
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("talonflow.config.memory_config.get_memory_config", return_value=config),
            patch("talonflow.agents.memory.updater.get_memory_data", return_value=data),
        ):
...
class TestUploads:
    def test_upload_files(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("talonflow.client.get_uploads_dir", return_value=uploads_dir), patch("talonflow.client.ensure_uploads_dir", return_value=uploads_dir):
    ...
    def test_upload_files_reuses_single_executor_inside_event_loop(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.client.get_uploads_dir", return_value=uploads_dir),
                patch("talonflow.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("talonflow.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("talonflow.utils.file_conversion.convert_file_to_markdown", side_effect=fake_convert),
    ...
    def test_delete_upload_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            with patch("talonflow.client.get_uploads_dir", return_value=Path(tmp)):
                with pytest.raises(FileNotFoundError):
...
class TestArtifacts:
    def test_get_artifact(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("talonflow.client.get_paths", return_value=paths):
...
class TestScenarioConfigManagement:
    ...
    def test_mcp_update_then_skill_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("talonflow.client.get_extensions_config", return_value=current_config),
                patch("talonflow.client.reload_extensions_config", return_value=reloaded_config),
            ):
            ...
            with (
                patch("talonflow.skills.loader.load_skills", side_effect=[[skill], [toggled]]),
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("talonflow.client.get_extensions_config", return_value=ext_config),
                patch("talonflow.client.reload_extensions_config"),
            ):
...
class TestScenarioAgentRecreation:
    ...
    def test_different_model_triggers_rebuild(self, client):
        ...
        with (
            patch("talonflow.client.create_chat_model"),
            patch("talonflow.client.create_agent", side_effect=fake_create_agent),
            patch("talonflow.client._build_middlewares", return_value=[]),
            patch("talonflow.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
        ):
...
class TestScenarioThreadIsolation:
    ...
    def test_uploads_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("talonflow.client.get_uploads_dir", side_effect=get_dir), patch("talonflow.client.ensure_uploads_dir", side_effect=get_dir):
...
class TestScenarioMemoryWorkflow:
    ...
    def test_memory_full_lifecycle(self, client):
        ...
        with patch("talonflow.agents.memory.updater.get_memory_data", return_value=initial_data):
            mem = client.get_memory()
        assert len(mem["facts"]) == 1

        with patch("talonflow.agents.memory.updater.reload_memory_data", return_value=updated_data):
            refreshed = client.reload_memory()
        assert len(refreshed["facts"]) == 2

        with (
            patch("talonflow.config.memory_config.get_memory_config", return_value=config),
            patch("talonflow.agents.memory.updater.get_memory_data", return_value=updated_data),
        ):
...
class TestScenarioSkillInstallAndUse:
    ...
    def test_install_then_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("talonflow.skills.loader.load_skills", return_value=[installed_skill]):
                skills_result = client.list_skills()
            ...
            with (
                patch("talonflow.skills.loader.load_skills", side_effect=[[installed_skill], [disabled_skill]]),
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("talonflow.client.get_extensions_config", return_value=ext_config),
                patch("talonflow.client.reload_extensions_config"),
            ):
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("talonflow.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("talonflow.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("talonflow.utils.file_conversion.convert_file_to_markdown", side_effect=Exception("conversion failed")),
            ):
...
class TestGatewayConformance:
    ...
    def test_install_skill(self, client, tmp_path):
        ...
        with patch("talonflow.skills.installer.get_skills_root_path", return_value=tmp_path):
            result = client.install_skill(archive)
    ...
    def test_update_mcp_config(self, client, tmp_path):
        ...
        with (
            patch("talonflow.client.get_extensions_config", return_value=ext_config),
            patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
            patch("talonflow.client.reload_extensions_config", return_value=ext_config),
        ):
    ...
    def test_get_memory_config(self, client):
        ...
        with patch("talonflow.config.memory_config.get_memory_config", return_value=mem_cfg):
            result = client.get_memory_config()
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("talonflow.config.memory_config.get_memory_config", return_value=mem_cfg),
            patch("talonflow.agents.memory.updater.get_memory_data", return_value=memory_data),
        ):
            result = client.get_memory_status()
...
class TestInstallSkillSecurity:
    ...
    def test_zip_bomb_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            from talonflow.skills import installer as _installer
            ...
            with (
                patch("talonflow.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("talonflow.skills.installer.safe_extract_skill_archive", side_effect=patched_extract),
            ):
    ...
    def test_invalid_skill_name_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("talonflow.skills.installer._validate_skill_frontmatter", return_value=(True, "OK", "../evil")),
            ):
    ...
    def test_existing_skill_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("talonflow.skills.installer._validate_skill_frontmatter", return_value=(True, "OK", "dupe-skill")),
            ):
    ...
    def test_invalid_frontmatter_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("talonflow.skills.installer._validate_skill_frontmatter", return_value=(False, "Missing name field", "")),
            ):
...
class TestConfigUpdateErrors:
    def test_update_mcp_config_no_config_file(self, client):
        ...
        with patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=None):
    ...
    def test_update_skill_no_config_file(self, client):
        ...
        with (
            patch("talonflow.skills.loader.load_skills", return_value=[skill]),
            patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=None),
        ):
            with pytest.raises(FileNotFoundError, match="Cannot locate"):
    ...
    def test_update_skill_disappears_after_write(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.skills.loader.load_skills", side_effect=[[skill], []]),
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("talonflow.client.get_extensions_config", return_value=ext_config),
                patch("talonflow.client.reload_extensions_config"),
            ):
...
class TestBugListUploadsDeadCode:
    ...
    def test_list_uploads_on_fresh_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("talonflow.uploads.manager.get_paths", return_value=mock_paths):
...
class TestBugAgentInvalidationInconsistency:
    ...
    def test_update_mcp_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("talonflow.client.get_extensions_config", return_value=current_config),
                patch("talonflow.client.reload_extensions_config", return_value=reloaded),
            ):
    ...
    def test_update_skill_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("talonflow.skills.loader.load_skills", side_effect=[[skill], [updated]]),
                patch("talonflow.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("talonflow.client.get_extensions_config", return_value=ext_config),
                patch("talonflow.client.reload_extensions_config"),
            ):

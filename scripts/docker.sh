...
COMPOSE_CMD="docker compose -p talon-flow-dev -f docker-compose-dev.yaml"
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"talonflow.sandbox.local:LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"talonflow.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  TalonFlow Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting TalonFlow Docker Development"
    echo "=========================================="
    echo ""
    ...
    # Set TALON_FLOW_ROOT for provisioner if not already set
    if [ -z "$TALON_FLOW_ROOT" ]; then
        export TALON_FLOW_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting TALON_FLOW_ROOT=$TALON_FLOW_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting TalonFlow.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
    ...
    echo ""
    echo "=========================================="
    echo "  TalonFlow Docker is starting!"
    echo "=========================================="
    echo ""
...
stop() {
    # TALON_FLOW_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" talon-flow-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting TalonFlow Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "TalonFlow Docker Management Script"
    echo ""
